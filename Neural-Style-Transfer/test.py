...
# # 사전 교육을 받은 ImageNet 가중치를 로드한 VGG19 모델 구축
model = vgg19.VGG19(weights="imagenet", include_top=False)

# Get the symbolic outputs of each "key" layer (we gave them unique names).
# # 각 "키" 계층의 기호 출력을 가져옵니다(우리는 그들에게 고유한 이름을 지정했습니다).

outputs_dict = dict([(layer.name, layer.output) for layer in model.layers])

# Set up a model that returns the activation values for every layer in VGG19 (as a dict)
# VGG19의 모든 계층에 대한 활성화 값을 반환하는 모델 설정(dict)
...
## TF를 추가합니다.함수 데코레이터에서 손실 및 그레이디언트 계산 컴파일하여 빠르게 만듭니다.
"""
...
for i in range(1, iterations + 1):
    ...
    if i % 1 == 0:
        ...
        print(img)
        fname = "1000,100/" + result_prefix + "_at_iteration_%d.png" % i
        fname = result_prefix + "_at_iteration_%d.png" % i
        keras.preprocessing.image.save_img(fname, img)

# feature_extractor.save('test1000.model', save_format="h5")
